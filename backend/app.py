...
def order():
    ...
    try:
        ...
        cursor.execute(
            ...
            (email, city, street, house, apartment, name, 'In processing')
        )
        conn.commit()
        return jsonify({"message": "The order has been successfully created!"}), 201
...
def check_order_status():
    ...
    if not email:
        return jsonify({"error": "You have to fill in your email"}), 400
